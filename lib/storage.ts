...
export class Bits {
  ...
  static assign<L extends string>(labels: readonly L[]) {
    ...
    class BitsProcess {
      ...
      get labels_off() {
        return BitsProcess.get_labels(BITMASK - this.target.bits)
      }
      get labels_on() {
        return BitsProcess.get_labels(this.target.bits)
      }
    ...
    labels.forEach((label, idx) => {
      ...
      Object.defineProperty(BitsBox.prototype, label, {
        enumerable: true,
        get: function (this: BitsBox): boolean {
          return Boolean(this.bits & bit)
        },
...
function to_url(o, base) {
  switch (base.constructor) {
    case String:
      return [encodeURIComponent(to_String(o, ''))]
    case Number:
...
function by_url(o, base) {
  switch (base.constructor) {
    case String:
      return to_String(decodeURIComponent(o[0]), '')
    case Number:
      return to_Number(decodeURIComponent(o[0]))
    case Boolean:
      return to_Boolean(o[0])
  }
  if (base instanceof Bits) {
...
export function pushState(o: VALUE_SETS) {
  defineUrlStore(pushStateStorage, o)
}
export function replaceState(o: VALUE_SETS) {
  defineUrlStore(replaceStateStorage, o)
}
function defineUrlStore(store: UrlStorage, o: VALUE_SETS) {
  defineStore<VALUE_SET, VALUE_SETS, string[]>(store, o, to_url, by_url)
  store.reset(o)
}
export function localStore(o) {
  defineStore<VALUE_TREE, VALUE_TREES, string | null>(LocalStorage, o, to_str, by_str)
}
export function sessionStore(o) {
  defineStore<VALUE_TREE, VALUE_TREES, string | null>(SessionStorage, o, to_str, by_str)
}
function defineStore<T, TT, P>(
  storage: Storage<P>,
  o: TT,
  to: (o: T, base: T) => P,
  by: (o: P, base: T) => T
...
class UrlStorage {
  ...
  static parse(href = BaseUrl(), base = BaseUrl()): UrlData {
    const { hash, search, pathname, username, password, protocol, host, hostname } = new URL(
      href,
      base
    )
    const data: UrlData = {
