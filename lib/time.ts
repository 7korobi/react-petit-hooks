...
export class Tempo {
  ...
  constructor(zero: number, now_idx: number, write_at: number, last_at: number, next_at: number, table: number[] | null = null) {
    if ( table ) {
      this.table = table
    }
    this.zero = zero
...
export function to_sec (str: string): number {
  let timeout = 0
  str.replace(/(\d+)([ヵ]?([smhdwy秒分時日週月年])[間]?(半$)?)|0/g, (_full, num_str: string, _fullunit, unit: string, appendix: string): string => {
